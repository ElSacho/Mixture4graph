        ...
        for i in range(n_nodes):
            for q in range(n_clusters):
                ...
                for j in range(n_nodes):
                    for l in range(n_clusters):
                        if j != i :
                            b = (pi[q,l]**X[i,j])**tau[j,l] * ((1-pi[q,l])**(1-X[i,j])) ** tau[j,l]
                            val_iq *= b
                tau[i,q] = priors[q] * val_iq
...
def appro_all_in_oneNP(tau, graph_edges, pi, priors, eps = 1e-04, max_iter = 50):
...
def main(X, n_clusters, max_iter):
    ...
    while current_iter < max_iter and not finished:
        ...
        # print("tau : \n",tau)
        # print("pi : \n",pi)
        new_tau = appro_all_in_oneNP(tau.copy(), X, pi.copy(), priors.copy())
...
def main_with_tau(X, tau, n_clusters, max_iter):
    ...
    while current_iter < max_iter:
        ...
        # print(priors)
        tau = appro_all_in_oneNP(tau.copy(), X, pi.copy(), priors.copy())
        # print(tau)
        # print("pi : \n",pi)
